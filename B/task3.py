...
X_cnn = X.reshape(-1, 28, 28)  # For CNN: reshape to 28x28 images
...
# ------------------------------
# Define CNN components and CNNClassifier
# ------------------------------
def relu(x):
...
plt.ylabel("True")
plt.savefig("plots/mlp_confusion_matrix.png")  # Save confusion matrix figure
plt.show()
...
plt.legend()
plt.savefig("plots/mlp_convergence.png")  # Save convergence figure
plt.show()
...
for epoch in range(cnn_epochs):
    ...
    cnn_test_losses.append(test_loss / X_cnn_test.shape[0])
    print(f"Epoch {epoch+1}/{cnn_epochs} - Train Loss: {epoch_loss:.4f} | Test Loss: {cnn_test_losses[-1]:.4f}")
...
plt.legend()
plt.savefig("plots/cnn_convergence.png")  # Save convergence figure
plt.show()
